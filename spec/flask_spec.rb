...
describe Mustermann::Flask do
  ...
  pattern '/<float:foo>' do
    it { should match('/42')  .capturing foo: '42'  }
    it { should match('/1.0') .capturing foo: '1.0' }
    it { should match('/.5')  .capturing foo: '.5'  }
    ...
    example { pattern.params('/42').should be == {"foo" => 42.0} }
    example { pattern.params('/.5').should be == {"foo" => 0.5 } }
